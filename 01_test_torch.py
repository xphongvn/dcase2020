...
if __name__ == "__main__":
    ...
    for idx, target_dir in enumerate(dirs):
        ...
        for idx, test_file in tqdm(enumerate(test_data), total=num_files):
            with torch.no_grad():
                pred_cpu = model(test_file).to('cpu').detach().numpy().copy()
            errors = numpy.mean(numpy.square(data - pred_cpu), axis=1)
            y_predict[idx] = numpy.mean(errors)
    ...
    result_path = "{result}/{file_name}".format(result=param["result_directory_torch"],
                                                file_name=param["result_file_torch"])
